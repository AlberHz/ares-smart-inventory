...
if 'CODIGO' not in df_stock.columns or 'CODIGO' not in df_mov.columns:
    st.error("No se detectó la columna 'CODIGO' en uno de los archivos cargados.")
    st.stop()
...
for col, val_default in [('DESCRIPCION', 'SIN DESCRIPCIÓN'), ('FAMILIA', 'SIN CLASIFICAR'), ('ALMACEN', 'GENERAL'), ('UM', 'UM'), ('COSTO', 0.0), ('STOCK', 0.0)]:
...
# 1. MAESTRO GENERAL
# ==============================================================================
df_codigos_stock = df_stock[['CODIGO', 'DESCRIPCION', 'COSTO', 'UM', 'FAMILIA', 'ALMACEN', 'STOCK']].copy()
...
mediana_var = df_evolutivo['Variacion_Porcentual'].iloc[1:].median() if len(df_evolutivo) > 1 else 0
...
if meses_historicos:
    ...
    auditoria_skus = []
    datos_mes_sel = saldos_por_mes_sku.get(mes_auditoria, {})
    ...
    df_auditoria = pd.DataFrame(auditoria_skus).sort_values(by='Valorizado_Cierre', ascending=False) if auditoria_skus else pd.DataFrame()
...
with pd.ExcelWriter(output, engine='openpyxl') as writer:
    ...
    if 'df_auditoria' in locals() and not df_auditoria.empty:
